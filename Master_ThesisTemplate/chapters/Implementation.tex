...
\begin{figure}[!htb]
	\centering
	\vspace{-1em}
	\makebox[\textwidth]{\includegraphics[scale=.50,width=\textwidth]{styles/system_architecture_new.pdf}}
	\vspace{-5em}
...
charts and code in order to detect information flow errors. The grammar of annotation language is represented as in Extended Backus Naur Form (EBNF) in Figure \ref{language grammar}\cite{ref_108_paul2015infoflow}. The following type face conventions have been used: Italic font for non-terminals, bold typewriter font for literal terminals including keywords.
 ...
 \begin{figure}[ht!]
 	...
 	\begin{tabular}{lll}
 		...
 		\footnotesize    	                   
 		\textit{Var\_Type}           &\footnotesize $::=$        &\footnotesize \textbf{authenticated}; \qquad \Comment{Newly added annotations to annotate variable types (Appendix  \ref{Source_Code_Editor_in_Xtext} line number 170)}\\
...
\begin{figure}[ht!]
	...
	\begin{tabular}{ll} 	         
	   \checkmark \footnotesize 
		 \circled{0} (INT)           &\footnotesize  $ \gamma  \vdash n :$ L    \qquad \Comment{L is attached to an integer value}\\ \\
		
		\checkmark \footnotesize 
		\circled{1} (VAR)            &\footnotesize  $ \gamma  \vdash x$ : H  $\ var$ \ \ \ if $\gamma(x)$ = H $\ var$  \Comment{H is attached to a varaiable}\\ \\
		
		\checkmark \footnotesize  \circled{2} (R-VAL)     &\footnotesize 
		\inferrule{\gamma \vdash e : \ $H$  \ var}
		{\gamma \vdash e : \ $H$  } \Comment{H is passed during a return statement}\\ \\  
		...
		{\gamma \vdash  e : \tau_{r}  \ ( $L$ )}  \qquad \Comment{Function: authentication, declassification or sanitization}\\ \\ 
		 ...
		 {\gamma \vdash e := e\textquotesingle: \ $H$ \ cmd } \Comment{H is passed during an assignment statement}\\ \\		 
		\footnotesize  \circled{5} (COMPOSE)   &\footnotesize 
		\inferrule{\gamma  \vdash c: \ $L$\ cmd \\\\
			\gamma  \vdash c\textquotesingle: \ $H$\ cmd }
		{\gamma  \vdash c ; c\textquotesingle: \ $H$ \ cmd } \Comment{L and H are passed during a composition statement}\\ \\     
		
		\footnotesize  \circled{6} (IF)        &\footnotesize 
		\inferrule{\gamma  \vdash e : \ $H$  \\\\
			\gamma  \vdash c: \ $ H$\ cmd    \\\\
			\gamma  \vdash c\textquotesingle: \ $H$\ cmd }
		{\gamma  \vdash if \ e \ then \ c \ else \ c\textquotesingle: \ $H$ \ cmd  } \Comment{H is passed during an if statement}\\ \\   
		
	\end{tabular}
	\caption{Typing rules for secure explicit and implicit information flow(\checkmark implemented)}
